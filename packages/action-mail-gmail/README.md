...
The leading and trailing whitespace is trimmed from action mail field `entities` and `variables`, e.g. `{     pay  }` is parsed as `{ pay: true }`; `{   one:   two  }` is parsed as `{ one: 'two' }`.
